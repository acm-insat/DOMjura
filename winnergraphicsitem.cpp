...
namespace DJ
{
    namespace View
    {
        ...
        WinnerGraphicsItem::WinnerGraphicsItem(QGraphicsItem *parent) : QObject(), QGraphicsItem(parent)
        {
            this->winner = "n2dg";
            this->reAddItems();
        }
        ...
        void WinnerGraphicsItem::reAddItems()
        {
            ...
            foreach (QString word, words)
            {
                ...
                proposedLine += word;
                int newWidth = fm.horizontalAdvance(proposedLine);
                if (newWidth <= myWidth)
                {
            ...
            foreach (QString line, lines)
            {
                ...
                textItem->setPos(centerWidth - fm.horizontalAdvance(textItem->text()) / 2, centerHeight + offsetFromCenter);
